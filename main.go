...
func main() {
	...
	if action == "closed" && prMerged == "true" {
		...
		if !deploycheck.CheckVersionAndHeoRevisionDiff(r, file) {
	...
	deploycheck.WriteOutput(outputFile, "COMPONENT", component)
	deploycheck.WriteOutput(outputFile, "ENVIRONMENT", environment)
	deploycheck.WriteOutput(outputFile, "VERSION", version)
	deploycheck.WriteOutput(outputFile, "IS_RELEASE", isRelease)
	deploycheck.WriteOutput(outputFile, "HEO_REVISION", heoRevision)
	deploycheck.WriteOutput(outputFile, "DEPLOYMENT_NEEDED", fmt.Sprintf("%t", needDeployment))
