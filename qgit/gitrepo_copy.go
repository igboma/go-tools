...
// 	//GetPRs() ([]PullRequest, error)
// 	GetChangedFiles(ref string) ([]string, error)
// 	GetChangedFilesEndingWithYAML(ref string) ([]string, error)
...
// func (gr *GitRepo) GetChangedFiles(ref string) ([]string, error) {

// 	// Fetch the remote branch to make sure the reference exists locally
// 	err := gr.Repo.Fetch(&git.FetchOptions{
// 		RemoteName: "origin",
// 		RefSpecs:   []config.RefSpec{config.RefSpec(fmt.Sprintf("+refs/heads/%[1]s:refs/remotes/origin/%[1]s", ref))},
// 		Auth: &http.BasicAuth{
// 			Username: "git", // GitHub ignores username but requires it
// 			Password: gr.Option.Token,
// 		},
// 	})
// 	if err != nil && err != git.NoErrAlreadyUpToDate {
// 		return nil, fmt.Errorf("failed to fetch remote branch %s: %w", ref, err)
// 	}
...
// 	// Resolve the comparison branch from the remote (origin)
// 	compareRef, err := gr.Repo.Reference(plumbing.ReferenceName("refs/remotes/origin/"+ref), true)
// 	if err != nil {
// 		return nil, fmt.Errorf("failed to resolve reference %s: %w", ref, err)
// 	}

// 	// Get the commit for the comparison reference
// 	compareCommit, err := gr.Repo.CommitObject(compareRef.Hash())
// 	if err != nil {
// 		return nil, fmt.Errorf("failed to get commit for ref %s: %w", ref, err)
// 	}

// 	// Get the commit for the current HEAD reference
...
// // GetChangedFilesEndingWithYAML filters changed files that end with .yaml.
// func (gr *GitRepo) GetChangedFilesEndingWithYAML(ref string) ([]string, error) {
// 	changedFiles, err := gr.GetChangedFiles(ref)
// 	if err != nil {
// 		return nil, err
// 	}
